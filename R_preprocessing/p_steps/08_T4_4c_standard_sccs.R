...
for (subpop in subpopulations_non_empty) {
  ...
  if (nrow(sccs_population) < 5) {
    ...
    ## fit model 
    unadjusted <- lapply(split(sccs_population, sccs_population$type_vax1), quietly(fit_unadjusted_standard_sccs))
    adjusted <- lapply(split(sccs_population, sccs_population$type_vax1), quietly(fit_adjusted_standard_sccs))
