...
for (subpop in subpopulations_non_empty) {
  ...
  if (nrow(sccs_population) < 5) {
    ...
    # some people are not eligible for the post vaccination SCRI because they don't have any FU after the 2nd dose risk period 
    # to be included, need to have a control period of at least 1. 
    if(any(sccs_population$c_length <=1)){
      
      message(paste("Note: no time after dose 2 risk period for ", sum(sccs_population$c_length <=1), "patients - they are deleted from post-vaccine SCRI!"))
      nrow0<-nrow(sccs_population)  
      sccs_population <- sccs_population[sccs_population$c_length >1,] 
      
      message(paste("Note: old row was", nrow0))
      message(paste("Note: new row is", nrow(sccs_population)))
      
    }
    
    # start at the first of control window or risk window (depending on design)
    sccs_population$ref_start <- as.numeric(sccs_population$days_vax1) 
