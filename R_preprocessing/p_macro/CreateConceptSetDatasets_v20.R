              ...
              if (df2 %in% dataset[[dom]]) {################### IF I GIVE VOCABULARY IN INPUT
                ...
                for (EAVtab_dom in EAVtables[[dom]]) {
                  if (df2 %in% EAVtab_dom[[1]][[1]]) {
                    ...
                    used_df[(stringr::str_detect(get(column_to_search), pattern)) & get(vocabulary[[dom]][[df2]]) == type_cod, c("Filter", paste0("Col_", concept)) := list(1, list(c(get(EAVtab_dom[[1]][[2]]), get(EAVtab_dom[[1]][[3]]))))]
                  }
                }
              }
            }
